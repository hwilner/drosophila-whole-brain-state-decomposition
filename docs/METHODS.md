...
- **Text rules.** Decodable tracked text is scanned with regular expressions for network addresses (`https?://`), email addresses, DOI-like identifiers (`10.\d{4,9}/...`), and outcome-style claims (phrases like "we found" or "this analysis showed"). `docs/INTRODUCTION.md`, which legitimately carries the project's verified reference list, is exempted from text scanning.
